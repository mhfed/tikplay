...
import { ListMusicIcon, ClockIcon, PlusIcon, CloseIcon } from './icons';
    ...
    <>
      ...
      <aside className={`mobile-sidebar${visible ? ' is-visible' : ''}`}>
        <div className="mobile-sidebar__header">
          ...
          <button className="mobile-sidebar__close" onClick={onClose} aria-label="Close">
            <CloseIcon size={20} />
          </button>
        </div>
            ...
            .map((p) => (
              <li key={p.id}>
                ...
                >
                  ...
                  {p.name}
                  {p.trackCount != null && (
                    <span className="mobile-sidebar__count">{p.trackCount}</span>
                  )}
                </button>
              </li>
            ))}
        ...
        <div className="mobile-sidebar__footer">
          <button className="mobile-sidebar__action" onClick={() => setShowAddPlaylist(true)}>
            <PlusIcon size={16} /> New Playlist
          </button>
          <button className="mobile-sidebar__action" onClick={() => setShowAutoRules(true)}>
