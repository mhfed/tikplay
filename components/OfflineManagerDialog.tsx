...
export default function OfflineManagerDialog({
  onClose,
      ...
      <DialogOverlay onClose={onClose} closeOnBackdropClick>
        <div className="flex w-[90vw] max-w-[400px] flex-col rounded-panel border border-line bg-elevated shadow-lg p-6">
          ...
          <p className="text-ink-secondary mb-6">
            Trình duyệt của bạn hiện không hỗ trợ chức năng tải nhạc offline
            (OPFS). Vui lòng thử lại trên Chrome, Safari, hoặc Edge phiên bản
            mới nhất.
          </p>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-hidden flex flex-col p-5 gap-5">
          ...
          <div className="rounded-lg border border-line bg-surface p-4">
            <div className="flex justify-between items-center mb-2">
              <div className="flex items-center gap-2 text-sm text-ink-secondary">
                ...
                <span>
                  Đã dùng {storageInfo ? storageInfo.usageFormatted : '...'} /{' '}
                  {storageInfo ? storageInfo.quotaFormatted : '...'} (
                  {usagePercent}%)
                </span>
              </div>
          ...
          <div className="flex items-center gap-2 text-sm bg-black/20 rounded p-2 border border-line-soft">
            {isOnline ? (
              ...
              <>
                <WifiOffIcon size={16} className="text-red-400" />{' '}
                <span className="text-red-400">Đang offline</span>
              </>
            )}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-ink-secondary truncate">
                      {track.author} • {formatSize(track.fileSize)}
                    </div>
                  </div>
